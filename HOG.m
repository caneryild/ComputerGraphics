...
function H=HOG(Im,B)
nwin_x=1;%number of HOG windows per bound box, more than 3 needs much computation power and memory
nwin_y=1;
[L,C]=size(Im);
H=zeros(nwin_x*nwin_y*B,1); % column vector 
m=sqrt(L/2);
if C==1 
    Im=im_recover(Im,m,2*m);%verify
    L=2*m;
    C=m;
end
Im=double(Im);
